...
var useCelsius = localStorage.getItem("celsius") !== null ? localStorage.getItem("celsius") === "true" : true;
var temper = {
  displayUnits: useCelsius ? "C" : "F",
  toFahrenheit: function toFahrenheit(t) {
    return Math.round(t * 9 / 5 + 32);
  },
  toCelsius: function toCelsius(t) {
    return Math.round((t - 32) * 5 / 9);
  }
};

var convertAll = function convertAll(toCelsius) {
  if (toCelsius) {
...
var updateAvg = function updateAvg() {
  dataset.avg = dataset.min.map(function (v, i) {
    return (v + dataset.max[i]) / 2;
  });
};
updateAvg();
...
var labels = [];
for (var i = 1; i <= Math.max(dataset.min.length, dataset.max.length); i++) {
  labels.push("Den " + i);
}

var gdmax = chartCanvas.getContext("2d").createLinearGradient(0, 0, 0, chartCanvas.width);
gdmax.addColorStop(0, "hsla(10,80%,70%,.2)");
gdmax.addColorStop(1, "hsla(10,80%,50%,0)");
var gdmin = chartCanvas.getContext("2d").createLinearGradient(0, 0, 0, chartCanvas.width);
gdmin.addColorStop(0, "hsla(210,80%,70%,.4)");
gdmin.addColorStop(1, "hsla(210,80%,50%,0)");

var tempChart = new Chart(chartCanvas, {
  type: 'line',
  data: {
    labels: labels,
    datasets: [{
  ...
  options: {
    ...
    tooltips: {
      ...
      displayColors: false,
      callbacks: {
        label: function label(i, d) {
          return tempChart.data.datasets[i.datasetIndex].label + ": " + i.yLabel + "°" + temper.displayUnits;
        }
      }
    },
    legend: {
...
var updateAll = function updateAll() {
  dataset.max.forEach(function (v, i) {
    if (v < dataset.min[i]) dataset.min[i] = v;
  });
  dataset.min.forEach(function (v, i) {
